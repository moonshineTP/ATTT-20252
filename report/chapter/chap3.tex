...
Hình~\ref{fig:pki-hierarchy} tóm tắt mô hình phân cấp phổ biến nhất trên Web PKI: niềm tin được kế thừa từ Root CA xuống Intermediate CA và chứng chỉ leaf.

\begin{figure}[htbp]
    \centering
    \reportfig[0.88]{fig_pki_hierarchy.png}
    \caption{Mô hình phân cấp Hierarchical PKI và chuỗi tin cậy}
...
\begin{figure}[htbp]
    \centering
    \reportfig{fig_digital_signature.png}
    \caption{Quy trình tạo và xác thực chữ ký số (hash + khóa bất đối xứng)}
...
Hình~\ref{fig:pki-components} mô tả luồng tương tác giữa các thành phần chính trong một triển khai PKI điển hình.

\begin{figure}[htbp]
    \centering
    \reportfig[0.88]{fig_pki_components.png}
    \caption{Các thành phần PKI và luồng cấp phát, lưu trữ, kiểm tra chứng chỉ}
    \label{fig:pki-components}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \reportfig[0.95]{fig_cert_lifecycle.png}
    \caption{Chu kỳ sống của chứng chỉ số trong hệ thống PKI}
